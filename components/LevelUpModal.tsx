    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
            ...
            {Array.from({ length: 30 }).map((_, i) => (
                <div 
                    ...
                    style={{
                        ...
                        backgroundColor: i % 2 === 0 ? 'var(--color-accent-1)' : 'var(--color-accent-2)',
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8 text-center max-w-sm w-full relative overflow-hidden animate-scale-in z-10">
        ...
        <div className="relative z-10">
          <h2 className="text-sm font-bold uppercase tracking-widest" style={{ color: 'var(--color-accent-2)' }}>Level Up!</h2>
          <div className="my-6 flex justify-center">
            <div className="relative">
                <div style={{ color: 'var(--color-accent-2)' }}>
                  <LevelUpIcon className="w-24 h-24 drop-shadow-lg" />
                </div>
                <span className="absolute -bottom-2 -right-2 text-white text-xl font-bold rounded-full w-12 h-12 flex items-center justify-center border-4 border-white" style={{ backgroundColor: 'var(--color-accent-2)' }}>{level.level}</span>
            </div>
          </div>
          ...
          <button
            ...
            style={{ backgroundColor: 'var(--color-accent-2)', color: 'var(--color-text-main)', '--tw-ring-color': 'var(--color-accent-2)' } as React.CSSProperties}
          >
            Awesome!
          </button>
        </div>
