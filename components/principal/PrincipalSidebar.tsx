...
interface PrincipalSidebarProps {
  user: Student;
  onLogout: () => void;
  activePage: PrincipalPage;
  setActivePage: (page: PrincipalPage) => void;
  isOpen: boolean;
  onClose: () => void;
}
...
export const PrincipalSidebar = ({ user, onLogout, activePage, setActivePage, isOpen, onClose }: PrincipalSidebarProps) => {
  return (
    <>
      <div 
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 flex-shrink-0 flex flex-col p-4 border-r z-40 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`} style={{ backgroundColor: 'var(--color-bg-main)', borderColor: 'var(--color-border)' }}>
        ...
        <div className="mt-auto">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={onLogout} className="p-2 rounded-full opacity-70 hover:bg-white/10 hover:opacity-100 transition-colors" aria-label="Logout">
