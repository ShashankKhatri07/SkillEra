...
interface PrincipalProfilePageProps {
  principal: Student;
...
export const PrincipalProfilePage = ({ principal, onUpdateUser, onUpdatePassword }: PrincipalProfilePageProps) => {
  const [bio, setBio] = useState(principal.bio || '');
  ...
  const canChangeName = () => {
      if (!principal.lastNameChangeDate) return true;
      const lastChange = new Date(principal.lastNameChangeDate);
  ...
  const daysUntilNextChange = () => {
      if (!principal.lastNameChangeDate) return 0;
      const lastChange = new Date(principal.lastNameChangeDate);
  ...
  const handleNameSave = () => {
    ...
    if (name.trim() && canChangeName()) {
        onUpdateUser({ ...principal, name: name.trim(), lastNameChangeDate: new Date().toISOString() });
    }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div className="lg:col-span-1 space-y-6">
          <ProfileCard user={principal} onUpdateUser={onUpdateUser} isAdmin={true} isEditable={true} />
           <Card title="Account Settings" style={cardStyle}>
            <div className="space-y-4">
                <div>
                    <label className="block text-sm font-bold mb-1">Display Name</label>
                    {isEditingName ? (
                        <div className="flex gap-2">
                            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-lg p-2" style={inputStyle}/>
        ...
        <div className="lg:col-span-2 space-y-6">
             <Card title="Professional Bio" style={cardStyle}>
                {isEditingBio ? (
                    <div>
                        <textarea
                            ref={bioTextareaRef}
                    ...
                    <div onClick={handleEditBioClick} className="cursor-pointer group">
                        <p className="whitespace-pre-wrap min-h-[50px]" style={{ color: 'var(--color-text-muted)' }}>
                            {principal.bio || <span className="opacity-60">Click here to add a bio...</span>}
                        </p>
